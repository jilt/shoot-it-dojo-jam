...
class PlayerTank extends GameEntity {
  ...
  private shoot = async () => {
    ...
    const bullet = new Bullet(shootingPosition, this._rotation, "player");
    GameScene.instance.addAndLoadEntity(bullet);

    // add effect
    const shootEffect = new ShootEffect(shootingPosition, this._rotation);
    GameScene.instance.addAndLoadEntity(shootEffect);
  };

  public damage = (amount: number) => {
    this._life -= amount;
    console.log(`Player health: ${this._life}`);
  ...
  public update = (deltaT: number) => {
    ...
    if (colliders.length) {
      // don't update position, but still update camera
    } else {
    ...
    const camera = GameScene.instance.camera;

    // 1. Calculate the ideal camera position (behind and above the tank)
    const cameraOffset = new Vector3(
      -Math.sin(this._rotation) * 3, // 3 units behind
      Math.cos(this._rotation) * 3,  // 3 units behind
      2.5                            // 2.5 units up
    );
    const cameraPosition = this._mesh.position.clone().add(cameraOffset);

    // 2. Set the camera's position
    camera.position.copy(cameraPosition);

    // 3. Determine the point for the camera to look at (slightly in front of the tank)
    const lookAtPoint = this._mesh.position.clone();
    lookAtPoint.z = 0.5; // Aim slightly down towards the tank's body
